...
**If stuck** — Transform "succeeded" but the three market fields are empty: the patch step is
missing; Transform rewrites in place and had nothing to rewrite. Three near-identical variants is
a prompt problem: the per-target instructions have to name the difference. A Function that fires
again after its own write has written outside the recursion guard. Reading the draft with GROQ
returns nothing under the published perspective; fetch it by id. Then `checkpoints/2-3.md`.
