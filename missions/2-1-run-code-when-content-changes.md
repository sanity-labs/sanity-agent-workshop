...
has `!defined(description.base)` in it. Then tell me exactly what to do in the Studio at
localhost:3333, with pnpm dev running, to make it fire on a real publish, and where to look for
the output. Do not deploy anything.
```

**Done when** — You publish a menu item (or run it locally) and the log shows what the Function
actually did.
...
The directory name must match the blueprint `name`. The emulator runs on port 8080 as part of
`pnpm dev`. Then `checkpoints/2-1.md`.
