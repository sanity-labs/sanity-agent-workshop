...
- **Auth is one organization token.** The attendee's KB is in their own org, so the KB endpoint
  takes `SANITY_ORGANIZATION_TOKEN`, the same bearer as the GROQ endpoint. A KB-mode endpoint with
  no readable KB is refused with JSON-RPC `-32005`; a token for the wrong org is a `403` naming
  the KB.
...
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HTTP `405` from the KB URL                       | It's the Context app's browser address (`context.sanity.io/<org>/knowledge-bases/kb…`), not an MCP endpoint. Use the `api.sanity.io/v1/context/organizations/<org>/mcp/<name>` URL, or append `?mode=knowledge_base&knowledgeBases=<kbId>` to the GROQ endpoint. |
| `403` naming a Knowledge Base                    | Wrong org's token at the KB endpoint. It takes `SANITY_ORGANIZATION_TOKEN`, the same as the GROQ endpoint.                                                                                                                                                       |
...
| Build still running at 1-3                       | Wait for **Entries up to date** before wiring anything; the build takes about ten minutes.                                                                                                                                                                       |
...
| KB answer has no citation                        | Not a code problem — an `instructions` lever on the KB. Point them at `kb/README.md` step 6.                                                                                                                                                                     |
